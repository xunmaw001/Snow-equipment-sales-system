<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="雪具分类" prop="xuejufenlei">
					<el-input v-model="ruleForm.xuejufenlei" placeholder="雪具分类" clearable  :readonly="ro.xuejufenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="雪具分类" prop="xuejufenlei">
					<el-input v-model="ruleForm.xuejufenlei" placeholder="雪具分类" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				xuejufenlei: [
					{ required: true, message: '雪具分类不能为空', trigger: 'blur' },
				],
			}
		};
	},
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuejufenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuejufenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
