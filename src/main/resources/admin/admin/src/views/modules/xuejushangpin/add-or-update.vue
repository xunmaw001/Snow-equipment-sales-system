	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="雪具分类" prop="xuejufenlei">
					<el-select :disabled="ro.xuejufenlei" v-model="ruleForm.xuejufenlei" placeholder="请选择雪具分类" >
						<el-option
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="雪具分类" prop="xuejufenlei">
					<el-input v-model="ruleForm.xuejufenlei"
						placeholder="雪具分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="优惠活动" prop="youhuihuodong">
					<el-select :disabled="ro.youhuihuodong" v-model="ruleForm.youhuihuodong" placeholder="请选择优惠活动" >
						<el-option
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="优惠活动" prop="youhuihuodong">
					<el-input v-model="ruleForm.youhuihuodong"
						placeholder="优惠活动" readonly></el-input>
				</el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				url: `option/xuejufenlei/xuejufenlei`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.xuejufenleiOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.shangpinxiangqing = this.ruleForm.shangpinxiangqing.replace(reg,'../../../springbootx9zss/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuejushangpin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuejushangpinCrossAddOrUpdateFlag = false;
						 this.parent.search();
