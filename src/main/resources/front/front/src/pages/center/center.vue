    ...
    <el-tabs tab-position="left" :style='{"background":"none"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="150px">
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xiaoshouyuan'" label="销售工号" prop="xiaoshougonghao">
            <el-input v-model="sessionForm.xiaoshougonghao" placeholder="销售工号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xiaoshouyuan'" label="密码" prop="mima">
            <el-input type="password" v-model="sessionForm.mima" placeholder="密码"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xiaoshouyuan'" label="销售姓名" prop="xiaoshouxingming">
            <el-input v-model="sessionForm.xiaoshouxingming" placeholder="销售姓名" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xiaoshouyuan'" label="照片">
            <el-upload
              class="avatar-uploader"
              :action="uploadUrl"
              :headers="headers"
              :show-file-list="false"
              :on-success="xiaoshouyuanzhaopianHandleAvatarSuccess">
              <img :style='{"border":"1px dashed #2886bc","cursor":"pointer","color":"#2886bc","borderRadius":"6px","textAlign":"center","width":"100px","fontSize":"32px","lineHeight":"100px","height":"100px"}' v-if="sessionForm.zhaopian" :src="baseUrl + sessionForm.zhaopian" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xiaoshouyuan'" label="性别">
            <el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
              <el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xiaoshouyuan'" label="电话" prop="dianhua">
            <el-input v-model="sessionForm.dianhua" placeholder="电话" ></el-input>
          </el-form-item>
          <el-form-item class="balance" :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xiaoshouyuan'" label="余额">
            <div :style='{"flexWrap":"wrap","display":"flex"}'>
				<el-input v-model="sessionForm.money" placeholder="余额" readonly></el-input>
				<div @click="dialogFormVisibleMoney = true" :style='{"border":"1px solid #2886bc","cursor":"pointer","padding":"0 30px","margin":"0 20px","color":"#2886bc","display":"inline-block","outline":"none","borderRadius":"30px","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}'>点我充值</div>
			</div>
		  </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
      if ('xiaoshouyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'xiaoshougonghao', null);
      }
      if ('xiaoshouyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'mima', null);
      }
      if ('xiaoshouyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'xiaoshouxingming', null);
      }
      if ('xiaoshouyuan' == this.userTableName) {
      ...
      if ('xiaoshouyuan' == this.userTableName) {
        this.$set(this.rules, 'xiaoshougonghao', [{ required: true, message: '请输入销售工号', trigger: 'blur' }]);
      }
      if ('xiaoshouyuan' == this.userTableName) {
        this.$set(this.rules, 'mima', [{ required: true, message: '请输入密码', trigger: 'blur' }]);
      }
      if ('xiaoshouyuan' == this.userTableName) {
        this.$set(this.rules, 'xiaoshouxingming', [{ required: true, message: '请输入销售姓名', trigger: 'blur' }]);
      }
			if ('xiaoshouyuan' == this.userTableName) {
    ...
    methods: {
      ...
      xiaoshouyuanzhaopianHandleAvatarSuccess(res, file) {
        if (res.code == 0) {
          if ('xiaoshouyuan' == this.userTableName) {
