...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","margin":"20px 7% 20px 14%","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 4px 0 0","alignItems":"center","display":"flex","height":"auto"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>雪具分类</div>
        <el-input v-model="formSearch.xuejufenlei" placeholder="雪具分类" clearable></el-input>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.xuejufenlei != '') searchWhere.xuejufenlei = '%' + this.formSearch.xuejufenlei + '%';
        this.$http.get('xuejufenlei/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/xuejufenleiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
