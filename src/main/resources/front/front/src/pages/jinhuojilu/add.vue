...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"10px auto","position":"relative","background":"none","display":"block"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","background":"linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(178,213,232,1) 100%)"}' label="进货日期" prop="jinhuoriqi" >
              <el-date-picker
                  format="yyyy 年 MM 月 dd 日"
                  value-format="yyyy-MM-dd"
                  v-model="ruleForm.jinhuoriqi" 
                  type="date"
                  placeholder="进货日期">
              </el-date-picker> 
          </el-form-item>
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.jinhuoriqi = this.getCurDate()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='jinhuoriqi'){
              this.ruleForm.jinhuoriqi = obj[o];
              this.ro.jinhuoriqi = true;
              continue;
            }
            if(o=='xinxibeizhu'){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('jinhuojilu/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post(table+`/update`,obj).then(res => {});
                          this.$http.post('jinhuojilu/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
